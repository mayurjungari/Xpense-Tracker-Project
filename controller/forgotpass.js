...
{  
    ...
    const sendSmtpEmail = {
        ...
        htmlContent: `Click the following link to reset your password: <a href="http://localhost:8000/resetPage/${uuidToken}">Reset Password Link</a>`,
      };
    ...
    <body>
        ...
        console.log('hello')
            function resetPassword(event) {
                ...
                axios.post('http://localhost:8000/resetpassword/${id}', { newPassword: newPassword })
                    .then((response) => {
                        console.log(response.data);
                        if(response.status==200) window.location.href='http://localhost:8000/'
                    })
                    .catch((error) => {
                        console.error(error);
                    });
